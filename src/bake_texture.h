...
/*
  These functions build and save texture maps to visualize a simplified
  intrinsic triangulation

Inputs:
  png_path: file to write texture to
  F: |F|x3 vertex-face adjacency list, where |F| is the number of faces in
       the simplified mesh
  F2V: |F| length list of lists, where F2V[f] gives you a list of indices in BC.
         ...
         coordinates in F[f,:]
  hit_mask: binary mask recording whether or not each pixel lies on the
              parameterized mesh
  pix_start: index of first image pixel stored in F2V lists (usually given by the
...
// image_rgba_buffer does not get modified, but the stbi_image_write function
// is not marked const
void bake_texture(
    const std::string & png_path,

    ...
    std::vector<std::vector<int>> & F2V)
{
    ...
    is_in_queue.setConstant(false);

    // add two ring face sides to the queue
    VectorXi fIdx;
    face_n_ring_faces(G,f_center,2,fIdx);
    for (int ii=0; ii<fIdx.size(); ii++)
    {
        int ff = fIdx(ii);
        for (int s=0; s<3; s++)
        {
            to_process.push({ff, s});
            is_in_queue(ff, s) = true;
        }
    }
    ...
    // cout << "perform " << num_flips << " local edge flips to Delaunay" << endl;
}
    ...
    const int & v_flatten)
{
    ...
    assert(is_boundary_vertex(G,v2fs,v_flatten)); // is boundary
    assert(!is_ear_vertex(G,v2fs,v_flatten)); // but not ear
    assert(is_one_ring_fs_valid(F,G,v2fs,v_flatten));

    // edge case: the boundary edge is a self edge (connecting vi to vi)
    {
        Vector2i fs_loop = v2fs.row(v_flatten).transpose();
        int vi, vj;
        get_face_side_vertices(F, fs_loop, vi, vj);
        if (vi == vj) // this is a self boundary edge
            return DOUBLE_INF;
    }
    ...
    cetm_flatten_boundary_vertex(l012, s012, u);
    if (isnan(u))
        return DOUBLE_INF;
    
    // once you go pass this point, the coarsening MUST not fail in the later procedure theoritically. For efficient implementation, let's change the edge lengths l and then change it back

    // get one-ring vertices
    VectorXi one_ring_v;
    vector<Vector2i> fs_of_one_ring_v;
    bool is_boundary_vertex; 
    vertex_one_ring_vertices(F,G,v2fs, v_flatten, one_ring_v, is_boundary_vertex, fs_of_one_ring_v);
    ...
    // could update angular coordinates (v_flatten & its one ring) here, but it shouldn't matter because we are approximating the cost anyway
        ...
        for (int ii=0; ii<fs_of_one_ring_v.size(); ii++)
        {
            ...
            for (int ch=0; ch<num_channels; ch++)
            {
                ...
                if (ii == 0) // the first v
                {
                    ...
                    Vector2i fs = fs_of_one_ring_v[ii];
                    assert(is_same_face_side(twin(G,fs), GHOST_FACE_SIDE));
                    double A_twin = get_angular_coordinate_range(G,l,v2fs,vj);
                    theta = A_twin - A(fs(0), fs(1)) + M_PI;
                    polar_to_cartesian(
                        l(fs(0), fs(1)), 
                        A_twin, 
                        logj_eji);
                }
                else if (ii == (fs_of_one_ring_v.size()-1)) // the last v
                {
                    // Vector2i fs_cw = one_ring_fs[ii-1];
                    Vector2i fs_cw = fs_of_one_ring_v[ii-1];
                    Vector2i fs_twin = next(next(fs_cw));
                    assert(is_same_face_side(twin(G,fs_twin), GHOST_FACE_SIDE));
                    double A_v = get_angular_coordinate_range(G,l,v2fs,v_flatten);
                    theta = A(fs_twin(0), fs_twin(1)) - A_v + M_PI;
                    polar_to_cartesian(
                        l(fs_twin(0), fs_twin(1)), 
                        A(fs_twin(0), fs_twin(1)),
                        logj_eji);
                }
                else // middle face sides
                {
                    // Vector2i fs = one_ring_fs[ii];
                    Vector2i fs = fs_of_one_ring_v[ii];
                    Vector2i fs_twin = twin(G, fs);
                    theta = A(fs_twin(0), fs_twin(1)) - A(fs(0), fs(1)) + M_PI;
                    polar_to_cartesian(
                        l(fs_twin(0), fs_twin(1)), 
                        A(fs_twin(0), fs_twin(1)),
                        logj_eji);
    ...
    Eigen::VectorXd & K)
{
    ...
    // update angular coordinates (v_flatten's one ring), do we need to update v_flatten as well?
    ...
    double cost = 0.0;
    {
        for (int ii=0; ii<fs_of_one_ring_v.size(); ii++)
        {
            ...
            for (int ch=0; ch<num_channels; ch++)
            {
                ...
                {
                    // Vector2i fs_cw = one_ring_fs[one_ring_fs_length-1];
                    Vector2i fs_cw = fs_of_one_ring_v[ii-1];
                    Vector2i fs_twin = next(next(fs_cw));
                    assert(is_same_face_side(twin(G,fs_twin), GHOST_FACE_SIDE));
                    double A_v = get_angular_coordinate_range(G,l,v2fs,v_flatten);

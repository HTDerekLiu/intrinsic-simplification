...
static void initialize_transport_cost(
    ...
    const double & w, // mixture weight (0~1) between curvature and area.
                      // 0 for pure curvature, 1 for pure area
    Eigen::MatrixXd & T)
{
    using namespace std;
    int nV = v2fs.rows();
    ...
    std::vector<std::vector<int>> & F2V)
{
    ...
    // check whether a vertex really exists
    ...
    Eigen::VectorXd K(nV);
    for (int v=0; v<nV; v++)
        K(v) = is_dead_vertex(v) ? 0 : gaussian_curvature_at_vertex(G,l,v2fs,v);

    // initialize transport cost
    Eigen::MatrixXd T;
    initialize_transport_cost(F, G, l, v2fs, K, mixture_weight, T);

    // initialize barycentric information if needed
    ...
    // create a time stamp vector for checking the time when v was inserted.
    // This is useful to avoid updating min heap
    ...
    while (v_min_heap.size() > 0 && num_removed < total_removal)
    {
        ...
        if (isinf(v_cost))
        {
            cout << "removed " << num_removed << "/" << total_removal
                 << " vertices and it cannot be coarsened further" << endl;
            return;
        }
        ...
        // if time stamp not match -> this element is an older element
        if (time_stamp != v_time_stamp(v))
            continue;
        ...
        // SKIP: if flattening failed, skip this vertex. (intrinsic data won't be
        //       changed in place either)
        if (isinf(v_cost))
        {

...
{
    std::vector<unsigned char> image_rgba_buffer;
    bake_texture(image_rgba_buffer, F, F2V, hit_mask, pix_start);
                  ...
                  int pix_start)
{
    ...
    for (int fIdx = 0; fIdx < F2V.size(); fIdx++){
        if (F2V[fIdx].size() > 0){
            for (auto idx : F2V[fIdx]){
                // idx - nV is the pixel index within the set of pixels
                // where hit_mask is true
                if (idx >= pix_start) {
                    // store (pixel index, face index)
                    coord_faces.emplace_back(idx - pix_start, fIdx);
                }
            }
        }
    }
    ...
    query_point_indices.setConstant(-1);    // initialize to -1, pixels outside faces
                                            // will remain -1
    int coord_idx = 0;
    ...
    // note that nV is the number of _original_ vertices
    int n_coarse_vertices = F.maxCoeff() + 1;
    ...
    //== write final pixel colors to buffer
    image_buffer.resize(4 * tex_size);
    for (int i = 0; i < tex_size; i++) {
        Vector3i color;
        if (query_point_indices(i) == -1
            || query_point_color_indices(query_point_indices(i)) == -1) {
            color = Vector3i{0, 0, 0};
        } else {
            color = colors.row(query_point_color_indices(query_point_indices(i)));
        }
        image_buffer[4 * i + 0] = (unsigned char) color(0);
        image_buffer[4 * i + 1] = (unsigned char) color(1);
        image_buffer[4 * i + 2] = (unsigned char) color(2);
        image_buffer[4 * i + 3] = (unsigned char) 255;
    }
}

...
static double geodesic_curvature_from_lengths(
     const Eigen::MatrixXd & l012)
{
    ...
    // note that since the input vertex is an boundary vertex so we only need to compute pi - anglesum for the geodesic curvature
    ...
    const Eigen::MatrixXd & l012)
{
    ...
    double gradient = M_PI - angles.sum();
    ...
    double & u)
{
    ...
    double Kv = geodesic_curvature_from_lengths(l012);
    int max_iter = 50;
    ...
    for (int iter=0; iter<max_iter; iter++)
    {
        ...
        Kv = geodesic_curvature_from_lengths(l012_scaled);

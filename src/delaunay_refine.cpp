                     ...
                     int max_iterations)
{
    ...
    auto twin = [&](Vector2i fs) -> Vector2i { return ::twin(G, fs); };

    // check whether a vertex/face still exists
    auto is_dead_face = [&](int iF) -> bool { return F(iF, 0) == GHOST_INDEX; };
    auto is_dead_vertex = [&](int iV) -> bool { return v2fs(iV, 0) == GHOST_INDEX; };

    // check whether edge is boundary
    auto is_fixed = [&](Vector2i fs) -> bool {
        return fs(0) == GHOST_INDEX || twin(fs)(0) == GHOST_INDEX;
    };
        ...
        while (!delaunay_check_queue.empty()) {
            ...
            if (!is_delaunay(G, l, fs) && !is_boundary_face_side(G, fs)) {
                if (is_diamond_convex(G,l,fs)) {
                    // Store neighboring faces, which will need to be checked for
                    // circumradius constraint. Storing them now saves us from
                    // potential reindexing problems
                    array<int, 2> neighboring_faces{fs(0), twin(fs)(0)};
                    if (is_dead_face(neighboring_faces[1]))
                        throw runtime_error("dead twin (1)");

                    flip_edge(fs, F, G, l, A, v2fs, BC, F2V);

                    if (is_dead_face(neighboring_faces[1]))
                        throw runtime_error("dead twin (2)");
    ...
    auto delete_nearby_vertices = [&](Vector2i fs1, Vector2i fs2) {
        ...
        // remove inserted interior vertices
        ...
        // recheck edges/faces inside of ball for Delaunay/circumradius conditions
        for (auto p : nearby_verts) {
            int iV = p.first;

            if (is_dead_vertex(iV)) continue;

            vector<Vector2i> outgoing_face_sides;
            vertex_one_ring_face_sides(G, v2fs, iV, outgoing_face_sides);

            for (Vector2i fs : outgoing_face_sides) {
                // Add edge for Delaunay check
                if (!fs_in_delaunay_queue(fs)) {
                    delaunay_check_queue.push(fs);
                    in_delaunay_queue(fs(0), fs(1)) = true;
                }

                // Add face for circumcircle check
                if (needs_refinement(fs(0)))
                    enqueue_face_circumradius(fs(0));
            }
        }
    };
    ...
    auto insert_circumcenter = [&](int iF) -> int {
        ...
        if (end_in_face) {
            ...
            return insert_ear_vertex({f_c, iS}, 0.5, F, G, l, A, v2fs, BC, F2V);
        }

    };

    // === Outer iteration: flip and insert until we have a mesh that satisfies both
    //                      angle and circumradius goals
    // static int iInsert = 0;
    do {

        // == First, flip to delaunay
        // cout << "flipping... queue has size " << delaunay_check_queue.size()
        //      << endl;
        flip_to_delaunay_from_queue();
        // cout << "done flipping... queue has size " << delaunay_check_queue.size()
        //      << endl;
        ...
        if (!circumradius_check_queue.empty()) {
            ...
            //   - If the area has changed since this face was inserted in to the
            //     queue, skip it. Note that we don't need to re-add it, because it
            //     must have been placed in the queue when its area was changed
            ...
            if (A == face_area(iF) && needs_refinement(iF)) {
                ...
                // resize buffer to fit two new faces, and mark them as unenqueued
                in_delaunay_queue.conservativeResize(F.rows(), 3);
        ...
        // (which can happen rarely due to numerics), but don't do this more than
        // a few times, to avoid getting stuck in an infinite loop when numerical
        // ultra-badness happens

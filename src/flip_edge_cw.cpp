    ...
    std::map<std::pair<int, int>, std::pair<int, int>> & fs_dict)
{
	...
	Vector2i s0_key, s1_key, s2_key, t0_key, t1_key, t2_key;
	{
		s0_key << s0;
		s1_key << s1;
		s2_key << s2;
    ...
    // #################################################
    // TODO: create operation history to undo edge flips
    // #################################################
	...
	{ 
        ...
        MatrixXd U;
        MatrixXi UF,UFF;
		flatten_diamond_mesh(G,l,s0,false,U,UF,UFF);
    ...
    // TODO: create operation history 
    // vIdx = np.array(vIdx, dtype=np.int32)
    // ophist = ("FLIP", fIdx, Ff, Elf, Af, gIdx, Gg, vIdx, v2fs[vIdx,:], bcIdx,
    //     BCbc, F2V_f0, F2V_f1)
}
...
{
	std::map<std::pair<int, int>, std::pair<int, int>> fs_dict;
	flip_edge_cw(s0,F,G,l,A,v2fs,BC,F2V,fs_dict);
}
